...
function btnChange(index,flag,speed){
    ...
    if(flag){
        ...
        else if(index >0 && index <=f_btn.length-1){
        ...
        else if(index == f_btn.length){
            indexs = f_btn.length;
            //wrapBox.style.top = (-height * 3 - fh) +"px";
            divMove(wrapBox,(-height * (f_btn.length-1) - fh),20*speed);
            nav_ul[index].style.color = 'red';
        }
        else{
            indexs = f_btn.length;
        ...
        if(index <= 0 ){
            ...
            indexs = f_btn.length;
            wrapBox.style.top = (-height * (f_btn.length-1) - fh) +"px";
            nav_ul[index].style.color = 'red';
        }
        else{
            indexs = f_btn.length;
...
function autoScroll(obj){
	animaltemp=$(obj).animate({
		marginTop: 160-obj.offsetHeight+"px"
	},2000)
}

function outScroll(obj){
	animaltemp.stop();
